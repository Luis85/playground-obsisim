...
- **Unchanged on purpose:** `CAMP_TILE`, `haulDistance`, `haulTicks`, `HaulCandidate`, `claimableAt`, `compareHaulCandidates`, `nextHaulTarget`. (`legProgress` was on this list and is **no longer exported** — Task 12 deleted the canvas's pixel-space copy of the leg interpolation, which was its last external caller, so the dead-code gate caught it and it was un-exported at source rather than baselined. `legPositionOf` is the exported way to place a hauler on its leg.) `haulTicks` is re-expressed in terms of `haulTicksBetween` but keeps its signature — `haulerCapacity` and the commute charge still measure from the camp.
...
- `destSiteId` **reserves** room at the destination from the moment it is chosen, and `nearestSiteWithRoom` counts reservations against capacity exactly as `claimableAt` counts claims against a building's buffer. A refund that eats reserved room puts the returning hauler straight back into the split-load case reservation exists to remove.
  ...
  **Honouring that is the caller's job, not the banking call's.** `addAt(site, …)` and `refundAt(site, …)` see a site's *physical* contents and know nothing about room another hauler has been promised — giving them reservation awareness would mean teaching the ledger about haul trips, which the rest of this design keeps out of it. So every caller, cancellations included, resolves its destination through `destinationFor` — the load's own **source site first** where that is still live and has room for all of it, then `nearestSiteWithRoom(…, heldAt, amount)` — with the **same reservation-aware `heldAt`** dispatch uses, and banks only into what that returns. The camp being unbounded is what makes that resolution always succeed.

  **Corrected 2026-08-10, after Task 8's review.** This sentence used to name `nearestSiteWithRoom` alone, which contradicted §2.5 step 4 and this plan's own tests for the path — both of which only have teeth against a source-first resolver, and neither of which the implementation could satisfy by following the prose. The code was written source-first and is right; the prose was wrong. The reservation-aware `heldAt` is what the paragraph was always about, and it binds under either resolver.
...
- [ ] **Step 2:** Storehouse glyph with a fill ring; `storing` and `waitingForInput` state colours; a hauler carrying **in** drawn distinguishably from one carrying **out**, so flow direction reads at a glance. **Read `haulPickedUp`, not `haulKind`** — the job kind is frozen at dispatch and stops describing the cargo exactly when §2.5's round trip works: a `supply` trip carrying collected output home would be drawn backwards, and that is the headline case in acceptance criterion 2. Also draw the `fetching` phase — and note it needs no per-phase geometry at all: every leg freezes `legFrom` and `legTo` (Task 6), so one interpolation over `legProgress` places a dot in any phase. That is strictly less code than the `CAMP_ANCHOR` special-casing it replaces.
  ...
  **`haulAt*` is only meaningful while `haulPhase === 'idle'`, and its failure mode is the quiet kind.** Only `cancel()` writes it, so mid-leg it holds the tile where the hauler last stood still — a *plausible* tile, not a sentinel. A bug that reads it unconditionally therefore draws a dot somewhere real and **looks fine**; the mirror pair `haulLegFrom*`/`haulLegTo*` reads `0,0` when cleared and is visibly wrong at a glance. Read the leg endpoints while a leg runs and `haulAt*` only when one is not.

  **This step also owes the layout half of Task 10 Step 3.** That step asked for all three states — mid-leg, idle at the camp, idle at a depot — covered in a *layout* test, and only the snapshot half was deliverable at the time, because `haulSpot` still had to be rewritten. It is rewritten here, so write those cases now: an idle hauler standing at a depot must be drawn **at the depot**, which is the case the camp-anchored geometry could never express and the one a player notices as a colonist teleporting home.
...
- [ ] **Step 5b: q5 — what does dispatch cost at scale?** A question this increment added and nobody has measured.

  `chooseJob` rebuilds **both candidate lists for every idle hauler**, and that is deliberate: it is what makes a same-tick dispatch visible to the next hauler without the `target.claimed += capacity` side-channel the old code used, and it is why every claim is reconstructible from live components. The cost is that a tick scales roughly as **idle haulers × buildings × sites × haulers** — `sitesHolding` calls `unclaimedAt` per site, the loop body calls it twice more, and each call rescans every trip. At 100 of each that is ~10⁸ trip checks; at a realistic 40/40/4 it is far less. Nobody knows which end of that range matters, because the numbers do not exist.

  Measure before touching it. Report wall-clock per tick at a **realistic** colony (roughly today's late-game: ~40 colonists, ~40 buildings, a few depots) and at a **stress** colony (100/100/8), alongside the same figures from `main` for the raw-producer control. Step 3's `frozenSteps` sentinel should be exercised at the stress size specifically — it is the instrument for this, and a sentinel only ever run on small colonies is not one.

  **Then decide with evidence.** If realistic-size cost is flat and only the stress case degrades, record the ceiling in §4 and leave the design alone — correctness bought cheaply is worth more than a speculative index. If a realistic colony is already paying, the fix named in review is to pre-index source claims by site and resource and reuse the candidate data within a tick, and that becomes an increment-8 item with a measured justification rather than a static complexity argument. **Do not optimise a correctness-critical dispatch path on a reading of the code alone.**
