...
it('a second hauler is not promoted to a building already being served', async () => {
  ...
  // starving building with an empty 12-unit tray, one topping-up building.
  // The first hauler is promoted to the starving one; the SECOND must not be,
  // because a 6-unit load is already claimed toward it.
  //
  // Assert the second hauler's TARGET, not just the first's: the bug is
  // invisible in the first dispatch, which is correct either way.
  //
  // Dropping `claims.input(...) === 0` alone must redden this and nothing else
  // — the two physical clauses survive that mutation and keep every other test
  // in the suite green, which is the shape the Global Constraints warn about.
});
...
- `BalanceResult` gains:
  - `transfers: number` and `transfersStaging` / `transfersDrain` — the two classes counted separately, because §4.2 must be able to say which half did the work. Read from `HaulTrip.staging` at the tick a transfer is dispatched, **not** re-derived from the route: a depot → camp move is legitimately either class (§2.2 makes the camp an ordinary site in the pull rule), so route-based attribution is wrong rather than approximate. Task 4 added the field for exactly this and nothing else.
    ...
    **These are EDGE counters and `HaulerTicks.transfer` is not one of them.** A review found the two conflated: `HaulerTicks.transfer` increments once per *active hauler-tick*, which is exactly right for a hauler-tick split and exactly wrong as a transfer count — it weights a long route more heavily than a short one, so a run with three slow transfers outscores one with six quick ones. Count a transfer where it is *dispatched*: the tick a hauler's trip goes from `idle` to `fetching` with `kind === 'transfer'`, incrementing `transfersStaging` or `transfersDrain` by that trip's `staging` flag. Both instruments are wanted and they answer different questions — "how much hauling did transfer consume" versus "how many transfers happened, of which class".

    **Read `staging` from the trip components, not from the snapshot, and do not publish it.** The harness builds the world itself, so it can sample `HaulTrip` directly. `ColonistSnapshot` does not carry `staging` and should not gain it: the snapshot is the *app's* read model, no UI surface consumes this field, and a published field that only a test reads is precisely the sort of thing that later looks load-bearing to someone deciding what they may change. Task 4's own rationale says the instruments are the field's only consumer — keeping it out of the snapshot is what makes that true rather than aspirational. Task 9 therefore does **not** publish it.
