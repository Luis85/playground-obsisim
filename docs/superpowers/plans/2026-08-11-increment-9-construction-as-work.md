...
- Demolishing a site refunds its delivered materials **to the camp** via `Stockpile.refund` — **landed in Task 3, verify rather than rewrite.** Not through `destinationFor`, which an earlier draft called for and which needs a live `HaulTrip` that a demolition does not have; spec §2.6 records the correction.
...
- **The save guard needs a CROSS-FIELD invariant, which no per-field check can express** (§2.10). `handleMoveBuilding` refuses to relocate a site, so the two countdowns are mutually exclusive in every save the engine writes — but a hand-edited or corrupt v7 file can carry `constructionTicks > 0` *and* `relocatingTicks > 0`, and `isTickCounter` accepts each of them independently. Loading it gives a building whose two countdowns both advance, with the relocation hidden behind `underConstruction` in the snapshot. Reject the record. This is the same class as the guard's existing colonist-reference rules: a per-record check that no single field can express.
  ...
  **STATE THE INVARIANT, DO NOT ENUMERATE THE SHAPES.** Two separate review rounds after Task 8 shipped found a third and a fourth record this guard accepts, and §2.7 already says enumerating one round at a time is not a method. The rule underneath all of them:

  > **A site holds goods in its in-tray and nowhere else, and nothing about it is running.** No output buffer, no stored goods, no active batch, no relocation.

  Every accepted-but-impossible record so far is one clause of that: both countdowns positive; a batch active or mid-progress; a **non-empty output buffer** — `collectCandidates` (`haul-dispatch.ts:93`) maps every building row to `buffered: buffer.total()` with no construction exclusion, so haulers bank a site's phantom output and mint goods from a corrupt file; and **non-empty `stored`** — `seedStoredGoods` (`restore.ts:236`) keeps it against the *finished* def's storage and seeds it as a stockpile site, so it counts in `colonyStock()` and is spendable by aggregate payments while `storeSitesOf` excludes the site and no hauler can ever reach it. That last one conserves goods; it is a consistency defect, not a minting one, and it is the weakest of the four.

  Write the guard as the invariant, with a fixture per clause. A fifth shape found later should then fail the existing rule rather than need a fifth patch.

  All of them are unreachable in ordinary play for the same reason — a site never produces, never stores, and is ordered empty — so this is corrupt-file defence, which is exactly what this guard is for.

  **Fix it in the guard, not in the dispatch path.** Excluding sites from `collectCandidates` would be the belt-and-braces version and is deliberately NOT done: it puts a construction term in the path this increment promised to leave alone, and the guard closes the only way the state can arise.

  **The production-state clause is the worst of them, and here is why.** `SavedBuilding` carries `progress: number` and `batchActive: boolean`, guarded only per-field (`save.ts:236-237` — `Number.isFinite` and `typeof === 'boolean'`), so `constructionTicks > 0` alongside `batchActive: true` passes every check here. Task 2b's exclusion makes `ProductionSystem` *skip* a site rather than reject its state, so the impossible batch sits frozen for the whole countdown and then **resumes at completion and yields output the site never consumed inputs for** — goods minted from a hand-edited file, through a path that looks like ordinary production. Require an under-construction building to have idle production state (`batchActive === false`, and `progress` at zero), and refuse the record otherwise.
