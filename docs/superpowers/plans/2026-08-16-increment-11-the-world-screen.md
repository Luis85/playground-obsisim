...
`PopulationPanel.vue` is `PopulationView.vue`'s headline block and colonist table, with `PopulationSummary` reused unchanged and every `<tr>` carrying `:data-test="`colonist-row-${w.id}`"` and `@click="ui.selectColonist(w.id)"`. Every label function (`ageLabel`, `commuteLabel`, `starvingLabel`, the class helpers) comes from `labels.ts`. `jobLabel` does NOT — as written, it is a local function inside `PopulationView.vue`, not in `labels.ts`; extract it there first (carrying its doc comment across) so both surfaces read one definition, per §2.7's "one figure, one derivation, two surfaces" and this repo's own convention that presentation lives in `labels.ts`, never copied between two views of the same column. And because the brief's headline-plus-table is, by this task's own wording, IDENTICAL to `PopulationView.vue`'s (not a narrower recut, unlike `ColonyPanel`'s deliberately different column count from `DashboardView`), pasting that markup a second time trips `check:quality`'s `cloneGroups`/`duplicatedLines` gate — factor the shared headline+table into one component (e.g. `PopulationRoster.vue`) that both `PopulationView.vue` and `PopulationPanel.vue` mount, rather than reshuffling markup until the detector stops noticing.
