...
describe('EfficiencySystem', () => {
  it('computes worker efficiency from hunger', async () => {
    ...
    const worker = spawnColonist(prep, getPrepResource(prep, IdCounter), { hunger: 75 });
  ...
  it('staffed worker consumes one tool for a 300-tick coverage that ticks down', async () => {
    ...
    const worker = spawnColonist(prep, ids, { buildingId: building.getComponent(Building)!.id });
    const world = await prep.prepareRun();
    await world.step();
  ...
  it('idle workers never consume tools', async () => {
    const prep = makePrep(2);
    const worker = spawnColonist(prep, getPrepResource(prep, IdCounter), {});
    const world = await prep.prepareRun();
    await world.step();
  ...
  it('covers exactly as many workers as there are tools', async () => {
    ...
    const first = spawnColonist(prep, ids, { buildingId });
    const second = spawnColonist(prep, ids, { buildingId });
    const world = await prep.prepareRun();
    await world.step();
  ...
  it('coverage follows the worker: replacements pay for their own tool', async () => {
    ...
    const building = spawnBuilding(prep, ids, { defId: 'forester', progress: 0, batchActive: false, col: 4, row: 1, relocatingTicks: 0 });
    const buildingId = building.getComponent(Building)!.id;
    const veteran = spawnColonist(prep, ids, { buildingId, toolTicks: 100 });
    const replacement = spawnColonist(prep, ids, {});
    const world = await prep.prepareRun();
  ...
  it('renews an expiring tool in the same tick (no untooled gap)', async () => {
    ...
    const ids = getPrepResource(prep, IdCounter);
    const building = spawnBuilding(prep, ids, { defId: 'forester', progress: 0, batchActive: false, col: 4, row: 1, relocatingTicks: 0 });
    const worker = spawnColonist(prep, ids, { buildingId: building.getComponent(Building)!.id, toolTicks: 1 });
    const world = await prep.prepareRun();
